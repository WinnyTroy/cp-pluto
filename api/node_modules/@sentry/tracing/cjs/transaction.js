...
class Transaction extends span.Span  {
   ...
   */
   finish(endTimestamp) {
    ...
    if (!this.name) {
      flags.IS_DEBUG_BUILD && utils.logger.warn('Transaction has no name, falling back to `<unlabeled transaction>`.');
    ...
    if (this.sampled !== true) {
      ...
      flags.IS_DEBUG_BUILD && utils.logger.log('[Tracing] Discarding transaction because its trace was not chosen to be sampled.');
    ...
    flags.IS_DEBUG_BUILD && utils.logger.log(`[Tracing] Finishing ${this.op} transaction: ${this.name}.`);
