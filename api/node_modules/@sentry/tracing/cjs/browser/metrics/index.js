...
function getBrowserPerformanceAPI() {
  return !utils.isNodeEnv() && global && global.document && global.performance;
}
...
function _trackCLS() {
  ...
  getCLS.getCLS(metric => {
    ...
    flags.IS_DEBUG_BUILD && utils.logger.log('[Measurements] Adding CLS');
...
function _trackLCP(reportAllChanges) {
  getLCP.getLCP(metric => {
    ...
    flags.IS_DEBUG_BUILD && utils.logger.log('[Measurements] Adding LCP');
...
function _trackFID() {
  getFID.getFID(metric => {
    ...
    var timeOrigin = utils$1.msToSec(utils.browserPerformanceTimeOrigin );
    var startTime = utils$1.msToSec(entry.startTime);
    flags.IS_DEBUG_BUILD && utils.logger.log('[Measurements] Adding FID');
...
function addPerformanceEntries(transaction) {
  ...
  flags.IS_DEBUG_BUILD && utils.logger.log('[Tracing] Adding & adjusting spans using Performance API');
  var timeOrigin = utils$1.msToSec(utils.browserPerformanceTimeOrigin);
    ...
    switch (entry.entryType) {
      ...
      case 'measure': {
        ...
        if (entry.name === 'first-paint' && shouldRecord) {
          flags.IS_DEBUG_BUILD && utils.logger.log('[Measurements] Adding FP');
          _measurements['fp'] = { value: entry.startTime, unit: 'millisecond' };
          _measurements['mark.fp'] = { value: startTimestamp, unit: 'second' };
        }
        if (entry.name === 'first-contentful-paint' && shouldRecord) {
          flags.IS_DEBUG_BUILD && utils.logger.log('[Measurements] Adding FCP');
  ...
  if (transaction.op === 'pageload') {
    ...
    if (typeof responseStartTimestamp === 'number') {
      flags.IS_DEBUG_BUILD && utils.logger.log('[Measurements] Adding TTFB');
      _measurements['ttfb'] = {
...
function _tagMetricInfo(transaction) {
  if (_lcpEntry) {
    flags.IS_DEBUG_BUILD && utils.logger.log('[Measurements] Adding LCP Data');
  ...
  if (_clsEntry && _clsEntry.sources) {
    flags.IS_DEBUG_BUILD && utils.logger.log('[Measurements] Adding CLS Data');
