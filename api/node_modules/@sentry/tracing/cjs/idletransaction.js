...
class IdleTransaction extends transaction.Transaction {
   ...
   */
   ...
   constructor(
    ...
    if (_onScope) {
      ...
      flags.IS_DEBUG_BUILD && utils.logger.log(`Setting idle transaction on scope. Span ID: ${this.spanId}`);
  ...
  /** {@inheritDoc} */
   finish(endTimestamp = utils.timestampWithMs()) {
    ...
    if (this.spanRecorder) {
      ...
      flags.IS_DEBUG_BUILD && utils.logger.log('[Tracing] flushing IdleTransaction');
    } else {
      flags.IS_DEBUG_BUILD && utils.logger.log('[Tracing] No active IdleTransaction');
    }
   ...
   */
   initSpanRecorder(maxlen) {
    if (!this.spanRecorder) {
      ...
      flags.IS_DEBUG_BUILD && utils.logger.log('Starting heartbeat');
      this._pingHeartbeat();
    }
   ...
   */
   _pushActivity(spanId) {
    this._cancelIdleTimeout();
    flags.IS_DEBUG_BUILD && utils.logger.log(`[Tracing] pushActivity: ${spanId}`);
    this.activities[spanId] = true;
    flags.IS_DEBUG_BUILD && utils.logger.log('[Tracing] new activities count', Object.keys(this.activities).length);
  }
   ...
   */
   _popActivity(spanId) {
    if (this.activities[spanId]) {
      flags.IS_DEBUG_BUILD && utils.logger.log(`[Tracing] popActivity ${spanId}`);
   ...
   */
   _beat() {
    ...
    if (this._heartbeatCounter >= 3) {
      flags.IS_DEBUG_BUILD && utils.logger.log('[Tracing] Transaction finished because of no change for 3 heart beats');
      this.setStatus('deadline_exceeded');
      this.finish();
    } else {
      this._pingHeartbeat();
   ...
   */
   _pingHeartbeat() {
    flags.IS_DEBUG_BUILD && utils.logger.log(`pinging Heartbeat -> current counter: ${this._heartbeatCounter}`);
    setTimeout(() => {
      this._beat();
    }, HEARTBEAT_INTERVAL);
  }
