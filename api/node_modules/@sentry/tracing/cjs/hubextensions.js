...
function sample(
  ...
  if (!isValidSampleRate(sampleRate)) {
    flags.IS_DEBUG_BUILD && utils$1.logger.warn('[Tracing] Discarding transaction because of invalid sample rate.');
    transaction.sampled = false;
    return transaction;
  }
  ...
  flags.IS_DEBUG_BUILD && utils$1.logger.log(`[Tracing] starting ${transaction.op} transaction - ${transaction.name}`);
  return transaction;
}
